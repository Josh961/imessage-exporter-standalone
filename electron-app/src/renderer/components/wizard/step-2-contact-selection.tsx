...
function getContactKey(contact: Contact): string {
  return [
    contact.type,
    contact.chatIds || contact.contact,
    contact.firstMessageDate || "",
...
export function Step2ContactSelection() {
  ...
  const [tooltip, setTooltip] = useState<{ participants: string; x: number; y: number } | null>(
    null,
  );
  ...
  return (
    <div className="rounded-3xl bg-white p-8 shadow-md ring-1 ring-slate-950/5">
      ...
      <p className="mb-6 text-center text-slate-600">
        Choose who you want to export messages with.
      </p>
          ...
          <div className="divide-y divide-slate-100">
            {filteredContacts.map((contact) => (
              <button
                ...
                aria-label={`Select ${getContactDisplay(contact)}`}
                  ...
                  <div className="mt-1 text-sm text-slate-500">
                    ...
                    {contact.firstMessageDate && contact.lastMessageDate && (
                      <span className="ml-2 text-slate-400">
                        {formatDateRange(contact.firstMessageDate, contact.lastMessageDate)}
                      </span>
                    )}
                  </div>
                </div>
      ...
      <button
        onClick={prevStep}
        className="mt-6 w-full rounded-xl border border-slate-300 px-6 py-3 font-semibold text-slate-700 transition-all hover:bg-slate-50"
      >
        Back
      </button>
      ...
      {tooltip && (
        <div
          className="pointer-events-none fixed z-50 rounded-lg bg-white px-3 py-2 shadow-lg ring-1 ring-slate-950/5"
          style={{ left: tooltip.x + 12, top: tooltip.y + 12 }}
        >
          <div className="mb-1 text-xs font-semibold text-slate-500">Participants</div>
          {tooltip.participants.split(",").map((participant) => (
            <div key={participant.trim()} className="text-sm text-slate-700">
              {formatPhoneNumber(participant.trim())}
            </div>
          ))}
        </div>
      )}
