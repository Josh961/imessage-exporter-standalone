...
function isValidDate(dateString: string): boolean {
  ...
  return (
    date instanceof Date && !isNaN(date.getTime()) && date.toISOString().slice(0, 10) === dateString
  );
}
  ...
  return (
    <div className="rounded-3xl bg-white p-8 shadow-md ring-1 ring-slate-950/5">
      ...
      <p className="mb-6 text-center text-slate-600">
        Choose the time period for messages with{" "}
      ...
      {suggestedRange && suggestedRange.first && suggestedRange.last && (
        <div className="mb-6 rounded-xl bg-sky-50 p-4 text-center text-sm text-sky-700">
          Messages available from {formatSuggestedDate(suggestedRange.first)} to{" "}
          {formatSuggestedDate(suggestedRange.last)}
        </div>
      )}
      ...
      <div className="space-y-4">
        <div>
          ...
          {suggestedRange?.first && (
            <button
              onClick={handleUseFirstDate}
              className="mt-2 text-sm text-sky-600 hover:text-sky-700 hover:underline"
            >
