      ...
      {steps.map((step, index) => (
        <div key={step.number} className="flex items-start">
          ...
          <div className="flex w-24 flex-col items-center">
            ...
            <span
              className={`mt-2 text-center text-xs font-medium ${step.number <= currentStep ? "text-slate-700" : "text-slate-400"}`}
            >
              {step.label}
            </span>
          </div>
          ...
          {index < steps.length - 1 && (
            <div
              className={`-mx-7 mt-5 h-0.5 w-14 ${step.number < currentStep ? "bg-sky-500" : "bg-slate-200"}`}
            />
          )}
        </div>
      ))}
